...
type (
	...
	Build struct {
		...
		Mode        string   // Docker build env mode
	}
...
func (p Plugin) Exec() error {
	...
	dockerconfig := defaultConfig
	_ = os.MkdirAll(dockerHome, 0600)
	if p.Login.Config != "" {
		dockerconfig = p.Login.Config
	}

	// Create Auth Config File
	path := filepath.Join(dockerHome, "config.json")
	err := ioutil.WriteFile(path, []byte(dockerconfig), 0600)
	if err != nil {
	...
	for _, img := range commandParse(p.Build.Dockerfile) {
...
func commandBuild(build Build) *exec.Cmd {
	args := []string{
		...
		"--rm=true",
		"--pull=true",
		"-f", fmt.Sprintf("%v/%v", build.Context, build.Dockerfile),
		"-t", build.Name,
	}
	...
	if build.Mode != "" {
		args = append(args, "--build-arg", fmt.Sprintf("MODE=%v", build.Mode))
	}
	...
	labelSchema := []string{
		...
		fmt.Sprintf("version=%s", build.Tags[0]),
		fmt.Sprintf("mode=%s", build.Mode),
	}
...
// commandParse 解析dockerfile
func commandParse(dockerfile string) []string {
	cmds, err := ParseFile(dockerfile)
	if err != nil {
		return nil
	}
	var froms []string
	for _, c := range cmds {
		if strings.ToLower(c.Cmd) == "from" && !strings.Contains(c.Value[0], "{") {
			froms = append(froms, c.Value[0])
		}
	}
	return froms
}
